...
type Server struct {
	...
	stopCh     chan struct{}
	connections map[quic.Connection]struct{}
...
func NewServer(cfg *config.Config) (*Server, error) {
	...
	return &Server{
		...
		connections:  make(map[quic.Connection]struct{}),
	}, nil
}
...
func (s *Server) BroadcastData(data *models.MetricsData) {
	...
	log.Printf("广播QUIC数据: %d 字节", len(data.String()))
}
...
// addConnection 添加连接
func (s *Server) addConnection(conn quic.Connection) {
	s.connMutex.Lock()
...
func (s *Server) removeConnection(conn quic.Connection) {
	s.connMutex.Lock()
	defer s.connMutex.Unlock()
...
// receiveData 接收数据
func (c *Client) receiveData(conn quic.Connection) {
