...
if [[ -f "test/quic_test.go" ]]; then
    if go build -o /dev/null test/quic_test.go 2>/dev/null; then
        echo -e "${GREEN}✅ quic_test.go 语法正确${NC}"
        ((VALID_FILES++))
    else
        echo -e "${RED}❌ quic_test.go 语法错误${NC}"
        ((INVALID_FILES++))
    fi
    ((TOTAL_FILES++))
fi
...
if [[ -d "pkg/quic" ]]; then
    ...
    for file in pkg/quic/*.go; do
        if [[ -f "$file" ]]; then
            if go build -o /dev/null "$file" 2>/dev/null; then
                echo -e "  ${GREEN}✅ $(basename "$file") 语法正确${NC}"
            else
